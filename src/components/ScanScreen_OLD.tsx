...
const ScanScreen = ({ onExit, reactionTimeOffset = 0 }: ScanScreenProps) => {
  ...
  const [trailSegments, setTrailSegments] = useState<Array<Array<{ x: number, y: number }>>>([])
  const [currentTrailSegment, setCurrentTrailSegment] = useState<Array<{ x: number, y: number }>>([])
  ...
  const startContinuousTrail = useCallback(() => {
    console.log('🚀 startContinuousTrail called, showTrail:', settings.showTrail)
    // Temporarily force trail to work regardless of settings
    ...
    const intervalId = setInterval(() => {
      const pos = mousePositionRef.current // Use mouse position instead of scanning cursor
      console.log('⏰ Trail interval tick, mouse pos:', pos)
      
      setCurrentTrailSegment(prev => {
        const newSegment = [...prev, { x: pos.x, y: pos.y }]
        console.log('📍 Current segment updated, length:', newSegment.length)
        return newSegment
      })
    }, 16) // Record trail points every 16ms for smooth continuous trail
    
    trailIntervalRef.current = intervalId
    console.log('✅ Trail interval created:', intervalId)
  }, [settings.showTrail])
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.code === 'Space') {
        e.preventDefault()
        console.log('🔴 Space pressed! isScanning:', isScanning, 'isSpacePressed:', isSpacePressed)
        if (!isSpacePressed && isScanning) {
          // Use current mouse position for trail start
          const startPos = { x: mousePosition.x, y: mousePosition.y }
          
          console.log('🎯 Setting start position to mouse:', startPos)
          setSpaceStartPosition(startPos)
          
          // Start a new trail segment with the initial mouse position
          console.log('🟢 Starting new trail segment at mouse position')
          setCurrentTrailSegment([startPos])
    ...
    const handleKeyUp = (e: KeyboardEvent) => {
      if (e.code === 'Space') {
        ...
        if (isSpacePressed && isScanning && spaceStartPosition) {
          ...
          // Complete the current trail segment and add it to trail segments
          if (currentTrailSegment.length > 0) {
            setTrailSegments(prev => [...prev, currentTrailSegment])
            setCurrentTrailSegment([])
          }
    ...
    const handleMouseDown = (e: MouseEvent) => {
      if (isScanning) {
        ...
        // Start a new trail segment with the mouse position
        console.log('🟢 Starting new trail segment (mouse) at:', startPos)
        setCurrentTrailSegment([startPos])
    ...
    const recordTrailToGrid = () => {
      // Record all current trail segment positions to the grid
      currentTrailSegment.forEach(pos => {
  ...
  }, [onExit, isSpacePressed, isMousePressed, isScanning, currentPosition, mousePosition, spaceStartPosition, dotSize, cols, rows, settings, reactionTimeOffset])
  ...
  return (
    <div className="fixed inset-0 bg-background">
      ...
      {/* Trail dots - Continuous trail effect */}
      {(settings.showTrail || true) && (trailSegments.length > 0 || currentTrailSegment.length > 0) && (
        <>
          {/* Render completed trail segments as continuous dots */}
          {trailSegments.map((segment, segmentIndex) => (
            <div key={segmentIndex}>
              {segment.map((pos, i) => (
                <div
                  key={`trail-${segmentIndex}-${i}`}
                  className="absolute pointer-events-none rounded-full"
                  style={{
                    left: pos.x - (dotSize * 0.6),
                    top: pos.y - (dotSize * 0.6),
                    width: `${dotSize * 1.2}px`,
                    height: `${dotSize * 1.2}px`,
          ...
          {/* Render current active trail segment as continuous dots */}
          {currentTrailSegment.map((pos, i) => (
            <div
              key={`current-trail-${i}`}
              className="absolute pointer-events-none rounded-full"
              style={{
          ...
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              ...
              <Button onClick={() => { 
                ...
                setTrailSegments([]) // Clear all trail segments on restart
                setCurrentTrailSegment([]) // Clear current trail segment
                setIsSpacePressed(false)
                setIsMousePressed(false)
                setSpaceStartPosition(null)
                setIsTransitioning(false)
